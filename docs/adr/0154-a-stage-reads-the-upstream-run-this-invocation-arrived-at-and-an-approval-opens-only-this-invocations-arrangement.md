...
- **`vespera label`, with the relevance floor set.** Recording answers arranges nothing and prints no gate line, so its closing line may neither point at "the gated line above" nor say an approval matches, since it checked none. It says what the next `vespera run` will do instead:

  > Every value the profile asks for is answered, including relevanceScoreFloor. Next: run vespera run, which arranges the documents and writes arrangement.html for you to approve.

  when `arrangementApproved` is unset, and

  > Every value the profile asks for is answered, and arrangementApproved holds "*held*". Next: run vespera run, which checks that value against the arrangement the documents are in then and asks for a new approval if they differ.
...
1. **A record of the runs this invocation minted or continued.** Keyed by stage, holding one id each. Every place a run is minted records it there at the moment `Ledger.startRun` returns, including stage 1's inline mint, which records into the context of the step it runs in on every call. It lives in `pipeline` (ADR-040). As built, the record is the job execution's own `ExecutionContext`, wrapped by `InvocationRuns` and handed to each run bean through `#{jobExecution.executionContext}` or `#{stepExecution.jobExecution.executionContext}`. It is not a bean, so `@CascadeSliceTest`'s import list gains nothing.
...
7. **Tests this decision moves.** The claims below were true of ADR-099's lookup or of `approvedArrangement(WalkId)`, and the decision changes them. Each is re-expressed with the code, keeping its claim except where noted, and none is weakened:
   ...
   - `ExtractionRunTest`, `ContentCensusRunTest`, `ContentCensusTaskletTest` and `ExtractionItemProcessorTest`. Only their fixtures change: they construct run beans directly and must hand them the upstream id the invocation's record would hold, instead of seeding a row for a lookup to find. `ContentCensusRunTest.namesTheExtractionRunAsItsUpstream`'s claim "stage 3 works out extraction's own run identity rather than being handed it" becomes "stage 3 names the extraction run this invocation recorded". `ExtractionRunTest.namesThePreviousStagesWorkAsWhatItRead`'s claim changes the same way: "works out the identity of the previous stage's work rather than being handed it" becomes "names the previous stage's work this invocation did, as the invocation recorded it".
   - Those four and `ByteLevelReductionTaskletTest` call stage 1's tasklet directly. Stage 1 now always records its run into the context of the step it is handed, so each is handed a real step (`InvocationRecordFixture.aStepOfAFreshInvocation`), a `ChunkContext` over a `JobExecution` built from `spring-batch-core`'s own types. `spring-batch-test` would do the same, but it is not in the pom, and adding it would want a decision (ADR-046). Each later run bean is handed `InvocationRecordFixture.recordOf` that same step, so what it names is what that invocation's stage 1 recorded, and never the run written last.
   - `NextActionTest`'s sixteen calls to the static `line(...)` pass whether the approval matches this invocation's arrangement explicitly. The value is true only in `withTheThresholdSetThereIsNothingLeftToSet`, whose claim is that the arrangement written over is the approved one, and false everywhere else, where nothing is approved or nothing is arranged. Their claims are unchanged.
   - `LedgerTest`'s two `latestRunFor` tests retire with the method.
   - `GenerationInvocationTest.stopsWhenTheApprovalMatchesTwo` retires, for the reason `ArrangementGateTest`'s "two" test does: the approval is matched against one arrangement, so it cannot match two. `ArrangementApprovalOverAReusedWalkTest` pins what an approval naming another arrangement does now.
   - `GenerationInvocationTest.keepsWhatAnEarlierInvocationWroteAndCarriesOnFromThere`, `GenerationInvocationTest.finishesTheClusterLeftBehindAndOnlyThenRecordsTheWorkAsDone` and `GenerationBreakerInvocationTest.doesNotClearTheCountForAClusterNothingCouldBeSentFor` reached 6b's "nothing in this cluster can be sent" branch (ADR-121) by inserting a `cluster` row that no document belongs to. Item 4's page, drawn on every invocation, refuses such an arrangement, as ADR-112's "the arrangement is total" requires. Their fixtures now reach the branch from rows an arrangement could record, and every claim is kept:
     - Each cluster holds exactly the document its count says, in the scoring run's membership. The two `GenerationInvocationTest` fixtures split the one real cluster into two of one document each. The breaker fixture gives the cluster that is to send nothing a document like every other.
     - That cluster's document is then rewritten in place, at the same length and with its last-modified time put back (`UnseenEditFixture`). The walk sees the same archive, so the approval stands and 6b really runs. No step converts or chunks the new bytes: steps whose work is recorded do nothing, and since ADR-152 the labelling page reads the extraction cache only. So 6b finds nothing cached under the file's current hash, and sends nothing for that cluster.
     - `finishesTheClusterLeftBehind…` lifts the obstruction by putting the document's own bytes back. That changes no run identity, so the same generation run finishes the job.
   - `GenerationInvocationTest.leavesTheStepOpenWhenAClusterCouldNotBeWrittenOver` passed without testing anything. It deleted the corpus's files, which gave the archive a new walk, so the gate was shut and 6b never ran. It now rewrites both documents in place by the same route, so its claims hold because 6b met the cluster and found nothing to send. Two claims pin that it ran: exactly one generation run is recorded over the corpus, and the model was asked nothing. Without them the claim that the work is not recorded as done would pass over an empty list of runs, which is the state a shut gate leaves.
   - **This depends on ADR-152, merged into this change from #293.** Before it, the labelling page converted and chunked every document it sampled, on every invocation. So a document rewritten in place was cached again under its new hash before 6b looked, and no realistic fixture could reach ADR-121's branch while an approval stood (measured on 2026-09-25). Wave 0b therefore carries #293's change and cannot land without it.
