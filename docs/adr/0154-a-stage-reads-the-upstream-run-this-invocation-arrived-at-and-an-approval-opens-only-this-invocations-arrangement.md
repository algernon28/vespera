...
7. **Tests this decision moves.** The claims below were true of ADR-099's lookup or of `approvedArrangement(WalkId)`, and the decision changes them. Each is re-expressed with the code, keeping its claim except where noted, and none is weakened:
   ...
   - `GenerationInvocationTest.keepsWhatAnEarlierInvocationWroteAndCarriesOnFromThere`, `GenerationInvocationTest.finishesTheClusterLeftBehindAndOnlyThenRecordsTheWorkAsDone` and `GenerationBreakerInvocationTest.doesNotClearTheCountForAClusterNothingCouldBeSentFor` reach 6b's "nothing in this cluster can be sent" branch by inserting a `cluster` row that no document belongs to. Item 4's page, drawn on every invocation, refuses such an arrangement, as ADR-112's "the arrangement is total" requires. **Open: no fixture yet reaches that branch from a state an arrangement really records.** Two were tried and measured on 2026-09-25, and both fail for a reason outside this record:
     - Splitting the cluster into two consistent clusters, and rewriting the second one's document in place with its length and timestamps kept, keeps the walk and the approval. But the relevance report step, which records no completion (ADR-118), converts and chunks every document it puts to a person on every invocation. So the rewritten text is cached before 6b reads it, and the document is sent.
     - A document that can no longer be opened is not reachable either. Deleting it is a different observation, so the approval names nothing (ADR-115). Making it unreadable in place does not work under root, and where it did, the same report step would fail the invocation on it (#289, ADR-152).
     
     `GenerationInvocationTest.leavesTheStepOpenWhenAClusterCouldNotBeWrittenOver` has the same cause and passes without testing anything. It deletes the corpus's files, which gives the archive a new walk, so its gate is shut and nothing reaches the branch. Its claims hold only because nothing ran.
