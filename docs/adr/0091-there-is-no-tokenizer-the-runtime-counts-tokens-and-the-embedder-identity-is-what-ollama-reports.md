...
Measuring them dissolved one of the two questions entirely. The tokenizer half's record is [`docs/research/embedding-tokenizer-sourcing.md`](../research/embedding-tokenizer-sourcing.md), and the claims below cite it rather than repeating it. The `/api/embed` measurements — the `truncate` contract, `dimensions` being honoured only at the top level, the re-normalised truncated vector — have **no separate record**: they are stated inline here, with their numbers, and that is the whole of them. Said plainly because a citation to a file nobody wrote is worse than an inline claim: it reads as corroboration that can be checked, and cannot be.
...
| instruction text | **`instruction=none`** where none was supplied; **`instruction=text:<the text>`** where one was | The sentinel and supplied text live in separate spaces, so an instruction whose text is literally `none` renders `instruction=text:none` and does not collide with the absence it would otherwise be indistinguishable from. A supplied-but-empty instruction is therefore `instruction=text:`, distinct from `instruction=none`, which ADR-084's "empty is a value, not an absence" requires. An omitted field would read as a field that failed to populate, which is what the refuse-a-blank discipline exists to catch. |
...
`/api/tags` carries the digest and the dtype as first-class fields in one call, which is the whole of what the runtime is asked for here — the dimension is the value sent, checked against what comes back, per the table above, and no endpoint is consulted for it. A small client over `RestClient`, beside the existing `DoclingClient`, reads those two. **Spring AI keeps the embedding calls** — that is what it is for, and [ADR-046](0046-the-pom-carries-what-a-recorded-decision-requires.md) already justified its presence.
