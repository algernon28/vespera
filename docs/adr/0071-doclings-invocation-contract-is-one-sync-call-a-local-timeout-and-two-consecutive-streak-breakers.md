...
The step is configured with `faultTolerant().skip(ServiceScopeFailureException.class)` — this is the reason Spring Batch is in the stack at all (`docs/architecture.md` §1.6). A generous `skipLimit` stays configured as a backstop against a slowly-degrading sidecar over a very long run, but it is **not** the circuit breaker: Spring Batch's `skipLimit` counts skips *cumulatively* across the whole step, so a corpus of 50,000 documents with ten sparse, harmless service-scope blips spread across a multi-day run would eventually trip it even though nothing was ever actually broken.
