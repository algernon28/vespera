# ADR-024 — Publication is terminal and one-shot

> **Superseded — see ADR-101.** [ADR-101](0101-the-run-ends-at-the-generated-documents-there-is-no-publication-stage-and-no-publication-target.md) **supersedes this record**: there is no publication to be terminal or one-shot.
...
| **Id** | ADR-024 |
| **Date** | 2026-08-20 |
| **Source** | decision-ledger row for ADR-024, compiled 2026-08-21 |

## Summary

One run per corpus; no re-publication, no idempotency-by-hash, no unpublish problem. Softens ADR-002 (Confluence's scope not fully settled).

## Cross-references

Extracted mechanically from the digest; the direction of an amendment is only as explicit as the summary above makes it.

- **Names**: [ADR-002](0002-confluence-is-the-publication-target.md)
- **Named by**: [ADR-002](0002-confluence-is-the-publication-target.md)
