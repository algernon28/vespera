...
**The rule is checkable by reading.** "Publication" as an adjective on the deliverable is the only use; any occurrence of *publish*, *publishes* or *published* in this repository's prose or identifiers is a defect, and the one that remains is the withdrawn `vespera publish` subcommand ADR-101 already scheduled for removal.
