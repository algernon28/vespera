...
- `synthesis/DifferenceHashTest` pins the hash against the golden values in research §6c:
  ...
  - a sample wider than 8 bits read by its high byte, a 16-bit RGB image hashing as its 8-bit twin does;
...
- `extraction/DocumentPictureTest` pins the place: the page and the four edges from the first `prov` entry, and no place where there is none, where the box or the first entry is `null`, or where the page number is missing.
...
- `pipeline/DeliverablePicturesInvocationTest` pins, through a whole invocation, that an `IMAGE` survivor whose conversion carries a unique picture shows its entry and no picture. It also pins (d) end to end: a survivor whose cached response has Docling's PDF shape, with a header picture cropped from pages 1 and 2 at one box and one chart, shows the chart under its entry and writes neither crop. The crops are different bytes 5 bits apart, so only their places can remove them, and their boxes' edges are chosen so that a place read with the page and an edge exchanged puts both crops on one page and keeps them.
