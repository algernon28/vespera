...
- Implementation versions follow `ScoringRun`'s precedent of naming every module the pass invokes, not only those it writes to: 6a is `of("synthesis", "extraction", "embedding", "pipeline")`, 6b is `of("synthesis", "embedding", "pipeline")`. No pom change is needed — `.mvn/scripts/implementation-versions.groovy` lists module directories, so `synthesis` gets its line the moment the package exists.
