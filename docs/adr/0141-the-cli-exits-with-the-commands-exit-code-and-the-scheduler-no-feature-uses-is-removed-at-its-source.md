...
- The process exit is pinned by `CliExitIT`, an integration test that launches the packaged jar and asserts it ends within a bound carrying the command's code — picocli's usage code for a rejected option, and success for `--version`. What is stated rather than tested is a last stage finishing and only then the process ending: a completing run shares the same `System.exit` line as a rejected command, differing only in the value picocli returns, so a hang on completion would be the very hang the test already fails on.
- The dependency half is verified rather than independently pinned. `ModuleBoundariesTest` and `VesperaApplicationIT` run the `-api` classpath and pass, and nothing asserts `spring-modulith-moments` is absent — the scheduler's absence is the mechanism of the decision, not a requirement of it, so a probe for it would pin an implementation detail rather than the decision.