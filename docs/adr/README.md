...
One file per architecture decision, ADR-001 through ADR-159.
...
| [ADR-054](0054-a-corpus-is-its-root-path-the-database-lives-in-a-configured-working-directory.md) | 2026-08-29 | A corpus is its root path; the database lives in a configured working directory |
...
| [ADR-059](0059-schema-version-is-one-row-per-module-checked-and-refused-independently.md) | 2026-08-29 | `schema_version` is one row per module, checked and refused independently |
| [ADR-060](0060-survivors-is-a-ledger-owned-item-reader-not-a-view.md) | 2026-08-29 | `survivors` is a `ledger`-owned item reader, not a view |
...
| [ADR-087](0087-clusters-are-modularity-communities-over-a-k-nearest-neighbour-graph-built-in-blocks.md) | 2026-09-06 | Clusters are modularity communities over a k-nearest-neighbour graph, built in blocks |
| [ADR-088](0088-the-relevance-threshold-is-read-off-a-stratified-sample-of-sixty-labels-and-an-unset-floor-does-not-stop-the-run.md) | 2026-09-06 | The relevance threshold is read off a stratified sample of sixty labels, and an unset floor does not stop the run *(amends ADR-028)* |
| [ADR-089](0089-a-stages-run-names-the-immediately-preceding-stages-run-upstream-because-verdicts-are-cumulative.md) | 2026-09-06 | A stage's run names the immediately preceding stage's run upstream, because verdicts are cumulative *(amends ADR-086)* |
| [ADR-090](0090-the-extractor-identity-is-the-sidecars-version-map-and-the-options-the-client-sends-never-its-url.md) | 2026-09-06 | The extractor identity is the sidecar's version map and the options the client sends, never its URL |
| [ADR-091](0091-there-is-no-tokenizer-the-runtime-counts-tokens-and-the-embedder-identity-is-what-ollama-reports.md) | 2026-09-07 | There is no tokenizer: the runtime counts tokens, and the embedder identity is what Ollama reports plus what we send *(amends ADR-044, ADR-084)* |
| [ADR-092](0092-the-seed-side-of-the-mismatch-comparison-is-measured-by-seed-extraction-under-the-measurement-run.md) | 2026-09-08 | The seed side of the mismatch comparison is measured by seed extraction, under the measurement run *(amends ADR-086)* |
| [ADR-093](0093-logging-is-explicit-and-process-scoped-console-plus-rolling-file-per-item-and-per-step-at-info.md) | 2026-09-08 | Logging is explicit and process-scoped: console plus rolling file, per-item and per-step at INFO, stage progress on a cadence |
| [ADR-094](0094-stage-1-decides-what-a-file-is-from-its-bytes-the-extension-may-only-narrow-within-that-class.md) | 2026-09-09 | Stage 1 decides what a file is from its bytes; the extension may only narrow within that class *(amends ADR-068)* |
| [ADR-095](0095-the-detected-format-is-a-stage-1-output-and-unrecognised-content-earns-no-verdict-until-the-mix-is-measured.md) | 2026-09-09 | The detected format is a stage-1 output under the run, and unrecognised content earns no verdict until the mix has been measured |
| [ADR-096](0096-k-retains-neighbours-regardless-of-distance-so-the-retained-edge-similarities-are-reported.md) | 2026-09-09 | k retains neighbours regardless of distance, so the retained-edge similarities are reported *(amends ADR-087)* |
| [ADR-097](0097-a-relevance-label-is-keyed-by-the-documents-path-and-the-seed-set-not-by-the-occurrence.md) | 2026-09-09 | A relevance label is keyed by the document path and the seed set, not by the occurrence *(amends ADR-088)* |
| [ADR-098](0098-getting-from-a-folder-to-a-curated-archive-is-four-invocations-and-the-operator-is-told-the-next-value-not-the-stage.md) | 2026-09-10 | Getting from a folder to a curated archive is four invocations, and the operator is told the next value rather than the stage |
| [ADR-099](0099-a-stages-upstream-run-is-looked-up-by-stage-and-walk-not-recomputed-and-two-candidates-stop-the-run.md) | 2026-09-11 | A stage's upstream run is looked up by stage and walk rather than recomputed, and two candidates stop the run *(amends ADR-048)* |
...
| [ADR-116](0116-a-runs-completion-is-recorded-per-step-because-several-steps-share-one-run.md) | 2026-09-14 | A run's completion is recorded per step, because several steps share one run *(amends ADR-115)* |
...
| [ADR-122](0122-the-vocabulary-binds-our-names-not-the-prose-rendered-for-an-outside-reader.md) | 2026-09-17 | The vocabulary binds our names, not the prose rendered for an outside reader *(amends ADR-052)* |
| [ADR-123](0123-an-answer-carrying-nothing-is-a-schema-violation-and-only-guaranteed-response-fields-are-dereferenced.md) | 2026-09-19 | An answer carrying nothing is a schema violation, and only guaranteed response fields are dereferenced *(amends ADR-108)* |
| [ADR-124](0124-both-fields-of-a-parsed-answer-are-required-and-missing-writing-is-a-schema-violation-rather-than-an-uncited-one.md) | 2026-09-19 | Both fields of a parsed answer are required, and missing writing is a schema violation rather than an uncited one *(amends ADR-108)* |
| [ADR-125](0125-an-absent-title-and-a-blank-one-are-told-apart-by-what-each-left-on-disk.md) | 2026-09-19 | An absent title and a blank one are told apart by what each left on disk, and ADR-124's owed guarantee is spent *(amends ADR-124)* |
...
| [ADR-144](0144-a-chunk-the-runtime-refuses-as-too-long-is-split-until-every-piece-fits.md) | 2026-09-24 | A chunk the runtime refuses as too long is split until every piece fits *(amends ADR-091)* |
| [ADR-145](0145-a-tables-cells-are-extracted-text-read-once-in-docling-reading-order.md) | 2026-09-24 | A table's cells are extracted text, read once, in Docling's reading order |
| [ADR-146](0146-spreadsheets-are-out-of-scope-and-stage-1-removes-them-with-a-verdict-of-their-own.md) | 2026-09-24 | Spreadsheets are out of scope, and stage 1 removes them with a verdict of their own *(amends ADR-057)* |
| [ADR-147](0147-the-docling-sidecar-is-a-derived-image-with-libreoffice-writer-and-impress-and-the-image-joins-the-extractor-identity.md) | 2026-09-24 | The Docling sidecar is a derived image with LibreOffice Writer and Impress, and the image joins the extractor identity *(amends ADR-090, ADR-100)* |
...
| [ADR-150](0150-a-pdfs-pictures-are-asked-for-as-embedded-pixels-and-a-picture-repeated-at-one-place-or-as-a-near-copy-is-furniture.md) | 2026-09-25 | A PDF's pictures are asked for as embedded pixels, and a picture repeated at one place or as a near-copy is furniture *(amends ADR-147, ADR-149)* |
...
| [ADR-159](0159-generation-asks-for-no-thinking-and-the-prompt-names-the-square-brackets-a-citation-is-written-in.md) | 2026-09-26 | Generation asks for no thinking, and the prompt names the square brackets a citation is written in *(extends ADR-108, ADR-109)* |
