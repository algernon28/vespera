# ADR-030 — *(parked)* Late chunking not adopted
...
| **Id** | ADR-030 |
| **Date** | 2026-08-20 |
| **Source** | decision-ledger row for ADR-030, compiled 2026-08-21 |

## Summary

Modest retrieval gain, but requires token-level embeddings Spring AI's `EmbeddingModel` doesn't expose. Revisit if boundary sensitivity dominates.
...
- **Names**: _none_
- **Named by**: _none_
- **Discussed in the digest at**: _the ledger row only_
