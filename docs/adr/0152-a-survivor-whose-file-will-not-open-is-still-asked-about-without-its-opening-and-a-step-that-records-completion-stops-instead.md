...
- **Keeps**: [ADR-050](0050-the-pipeline-has-exclusive-access-to-the-corpus.md) (the walk carries no concurrency defences), [ADR-104](0104-the-surviving-originals-stay-where-they-are-and-the-deliverable-references-them.md) as amended by ADR-149 §10 (6b checks no original and records no verdict for a missing one), [ADR-133](0133-the-exemplars-one-call-sent-are-recorded-and-a-cluster-file-numbers-its-membership-from-that-record.md) (an exemplar 6b cannot reach is left out of the call, and the call's exemplars are recorded), [ADR-115](0115-a-re-walk-that-observed-nothing-new-is-discarded-and-a-run-is-continued-under-its-own-id.md) and [ADR-116](0116-a-runs-completion-is-recorded-per-step-because-several-steps-share-one-run.md) (a finished step does no work, and an unfinished one is continued under its own id), [ADR-118](0118-the-answers-a-person-gave-never-join-a-runs-identity-so-the-two-steps-that-read-them-record-no-completion.md) (the labelling step records no completion), and [ADR-088](0088-the-relevance-threshold-is-read-off-a-stratified-sample-of-sixty-labels-and-an-unset-floor-does-not-stop-the-run.md) (the stratified sample, drawn from the run id).
...
| relevance floor (5e) | nowhere | | no (ADR-118) | not affected |
...
- **6b tolerates, is unchanged, and is an exception to the first rule above that this record leaves standing.** 6b records completion, per cluster and for the step, and it uses the file for what it records: an exemplar's opening chunk is what the synthesis doc is written from. Where `openingChunkOf` cannot hash an exemplar's file, it logs a warning naming the file and leaves that document out of the cluster's call. If any other document of the cluster opened, the call is made and the cluster is written without it. The `synthesis_doc` row then seals that cluster. `GenerationTasklet` skips every cluster that already carries one (ADR-111, ADR-115), so the repair pass only ever returns to a cluster with no such row: one whose answer was turned down, or one nothing could be sent for. A cluster written while one exemplar's file would not open therefore stays written without that exemplar for good, including after the file opens again. Only a cluster in which no document opened is left unwritten, and the step unfinished, as ADR-111 and ADR-121 already say.

  This tolerance was decided before this record. ADR-133 accepted the drop and chose to record it rather than remove it. ADR-149 §9 and §10 applied the same rule to the pictures read at tree-writing time, and ADR-104 records no verdict for the file. This record does not reopen any of them. The exception can stand for three reasons, and none of them holds for the four steps that stop:
  - **A synthesis doc is prose for a reader, not something a later step computes from.** Vectors, scores, membership and the cluster label are each read by a later step. The synthesis doc is read only to write the tree and to count what was written.
  - **The document is not lost from its cluster.** Its membership row is untouched, and the cluster file still lists it and links to its original (ADR-104, ADR-149 §10).
  - **The omission is on record and on the page.** `call_exemplar` records exactly which documents the call carried, and the cluster file numbers those first (ADR-133). The call carried fewer documents than the cluster holds, so the page says *"Written from the first k of the n documents in this group."* What neither the page nor the ledger says is *why* the document was left out. ADR-133 leaves that question open deliberately, and today the reason is only in the warning `openingChunkOf` logs.
...
- A survivor's bytes are rewritten in place with its size and modified time kept, so the walk is unchanged, and the next invocation must succeed without adding a conversion to the cache, show §3's fallback for that survivor, and log a warning naming its file (§3). This one runs everywhere.
- Embedding scoring meets a file that will not open while its step is unfinished. The invocation must fail with an error that names that file, and once the file opens the next invocation must succeed and write the page (§4). Guarded the same way as the first test.
