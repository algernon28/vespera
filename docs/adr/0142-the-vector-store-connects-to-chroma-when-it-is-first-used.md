...
- **Follows**: [ADR-141](0141-the-cli-exits-with-the-commands-exit-code-and-the-scheduler-no-feature-uses-is-removed-at-its-source.md), whose exit code a command could not return while the context failed to start.
...
- `CliExitIT` drops the `type=none` workaround and points the jar at a Chroma on a closed port, so it holds whether or not the machine running it has a Chroma sidecar up. `VectorStoreIsReachedOnFirstUseTest` starts the whole application in-process the same way, and pins both halves: the context starts, and asking for the vector store is what tries to connect — a refused connection rather than a missing bean, which is what separates deferring the store from removing it.
- `VesperaApplicationIT` still starts against a real Chroma through Testcontainers; it no longer exercises the connection, since nothing builds the bean.
