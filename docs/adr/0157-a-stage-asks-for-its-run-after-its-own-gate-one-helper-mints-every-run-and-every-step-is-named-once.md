...
That makes the rule a property of each call site rather than of wiring, so it is pinned by behaviour at every gate (Tests below), not by the shape of a bean. When this record was written, #309 was the one gate where the code already broke the rule. [ADR-160](0160-the-relevance-report-asks-both-seed-usability-questions-before-it-reaches-the-scoring-run.md) closed it: the relevance-report step now asks `StageFiveGates.modelSeedWalkUsable` before it calls `embeddingScoring()`. ADR-160's `ArrangementRun` is `StageRuns.arrangement()`; the claim holds unchanged. The one check a scoped constructor gave for free is kept as an input check instead: an accessor whose input is a gate's value throws `IllegalStateException` if called while that value is absent. So a caller that forgets its gate fails loudly, before `startRun`, rather than minting.
...
**Two orderings move.** The first is by a statement: a step's "finished under run …" line is logged at the end of `work`, just before `once` writes the completion row, where today it is logged just after. The text of the line does not change, and nothing reads between the two.

The second is generation's, and it was found in the implementing change. On `5cba9ef`, `GenerationTasklet` called `ledger.finishStep` and then wrote the deliverable tree. Under `once` it writes the tree inside `work`, so the completion row is recorded after the tree is written, as every other step's is. This changes no outcome. The step runs in one transaction, and ADR-111 measured that an exception leaving `execute` rolls that transaction back. So on `5cba9ef` a tree write that threw already took the completion row with it, and the next invocation did the step again. The new order keeps it: the step is recorded finished only once the tree it hands the operator is written. That now follows from the order of the statements, and no longer depends on the rollback.
...
Seven of these values are also stage names in `StageModules`. They are two persisted facts that happen to be equal: a `finished_step.step` value and a `run.stage` value. Each is declared in its own table, and neither is defined as the other. Every `STEP`, `STEP_NAME`, `SIGNATURE_STEP` and step-naming `STAGE` constant goes, together with the `"census"` literal. After the change, no `pipeline` source outside `StageModules` and `StepNames` holds a string literal naming a persisted stage or step. Module names are excepted. `extraction` is a module as well as a stage and a step, and `pipeline/package-info.java` names it as a module in `allowedDependencies`, which is not a use of the stage's or the step's name. Wherever a name is used, including the three chunk-step configurations, the `stepFinished` and `finishStep` calls, and `StageRuns`, it is read from one of those two tables.
...
- **Operator text.** Every log line and gate sentence keeps its text and its logger. Only §5's two reorderings move. The moved line's text does not change, and generation's deliverable is written before its completion row rather than after it, which changes no outcome. The `IllegalStateException` texts that name a deleted class change (§3). They are defect messages that no gate path reaches.
...
- **The usable-seed gate with a model named is pinned by ADR-160, not here.** On `5cba9ef`, with a model named and no seed producing text, `RelevanceReportTasklet` checked only `StageFiveGates.modelAndSeedWalk` and then reached the scoring run. That minted a `seed-measurement` run and an `embedding-scoring` run behind ADR-083's gate, and the invocation still exited 0. This was measured and filed as [#309](https://github.com/algernon28/vespera/issues/309). `SeedExtractionInvocationTest` pins "no run" for that gate only with no model named, which is why it never showed. This record left the gate unpinned, because the fix changes which sentence the step logs, and that was #309's to decide. [ADR-160](0160-the-relevance-report-asks-both-seed-usability-questions-before-it-reaches-the-scoring-run.md) decided it and landed before this change did. In this record's shape, the step asks `StageFiveGates.modelSeedWalkUsable` and calls `stageRuns.embeddingScoring()` only past it. `RelevanceReportMintsNothingBehindTheUsableSeedGateTest` pins that gate and ADR-155's the same way `NoRunBehindAShutGateTest` pins the others: every earlier gate is open, and only the runs in front of the gate remain. So `NoRunBehindAShutGateTest` does not repeat the case.
...
**Checked once the change exists, and written then, green:** `RunsAndNamesHaveOnePlaceTest` holds two guards over `pipeline`'s source, with comments stripped. The first checks that no `pipeline` source names `ObjectProvider<` of a type whose simple name ends in `Run`. The second checks that no `pipeline` source outside `StageModules` and `StepNames` holds a string literal naming a persisted stage or step. `package-info.java` is exempt from the second, because it holds module names (§7). Neither guard can pass on `5cba9ef`, so neither is in the tree before the change it checks. Each one first checks that its scan finds something it must find, so a scan that stopped finding anything fails rather than passing.
