...
| **Id** | ADR-048 |
| **Date** | 2026-08-21 |
| **Source** | decision-ledger row for ADR-048, compiled 2026-08-21 |

## Summary

Two distinct identities: walk (owns occurrences, filesystem-scoped) and run (owns verdicts, one stage/one config, chained to upstream run ids and implementation version). Continuation vs. minting rule defined.
