...
| Extraction serving engine (reference)  | One hosted model (OpenAI), configured for a separate run — a confirmation reference, never an in-pipeline fallback. There is no extraction-engine bake-off                                 | ADR-012, ADR-013, ADR-072 |
