...
/**
 * One group's worth of call (ADR-108, ADR-110): what it is called, what it sits under, and the
 * documents it is written from.
 *
 * <p>Handed over whole rather than gathered here. Everything in it is read where the passes are
 * assembled — this module may reach neither the table the groups live in nor the one the chunks do —
 * which is what leaves the call itself testable with no database under it.
 *
 * @param label what stage 6a named the group, which is what the call tells the model it is reading
 * @param seedPath the seed document whose partition the group sits in, named so the model knows what
 *     the group is a group of
