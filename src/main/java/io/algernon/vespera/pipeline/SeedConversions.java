 ...
 *
 * <p>Held in one place because the two passes that call it — the seed step and the scoring tasklet,
 * the latter converting a survivor's own file under the same byte-level detection wherever gate 3's
 * recorded {@code detected_format} is not at hand — would otherwise each carry the same five lines,
 * and changing how a format is found would mean editing two files to keep one answer. The relevance
 * report called this too, once; ADR-152 moved its preview onto the extraction cache alone, with no
 * conversion of its own, so it is no longer a caller.
 */
