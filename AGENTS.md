...
**One defect is known and open against what ships**: `Deliverable` escapes the pipe, the brackets and the backslash and folds a line break, but escapes neither `<` nor `&`, so a document's own title reading `<b>Retrofits` is inline HTML to a renderer and everything after it on the page is bold ([#251](https://github.com/algernon28/vespera/issues/251)). It is of #246's family — untrusted text out of the archive changing the page it is listed on — and it is milder: the values are names rather than markup, and GitHub sanitises what it serves. ADR-134 records it as the one question it declined to settle, because what it wants first is a measurement of which renderers the tree is actually read with. The defect that was — a citation's ordinal and the membership list's numbering were not the same numbering wherever a member was dropped between the cluster and the call, so a citation could lead to the wrong document ([#236](https://github.com/algernon28/vespera/issues/236)) — is closed by ADR-133: `synthesis`'s fourth table records which documents each call carried under which ordinal, and a cluster file numbers its membership from that record, sent first in citation order, rather than deriving score order a second time.
