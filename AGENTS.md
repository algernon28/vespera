...
**`docs/adr/`** holds 129 decisions, ADR-001 to ADR-129, and two things about it are invisible from the files:
...
**The current map is [issue #151](https://github.com/algernon28/vespera/issues/151), the stage 6a/6b slice**, charted on 2026-09-12 — the seventh, after six closed one per slice, ending with the stage 5 slice's [#78](https://github.com/algernon28/vespera/issues/78) on 2026-09-09. **Its route is walked**: every decision ticket is closed, and the hand-off spec it was charted for is [#175](https://github.com/algernon28/vespera/issues/175), which broke the build into its own tickets. The map stays open until it is closed with a retrospective; its body carries the destination, what was settled and what is deliberately out of scope, and each closed ticket's resolution comment carries the decision itself. Stage 7 never had a map and never will — ADR-101 struck it.

**What is takeable is not on the map.** With the cascade built, open work is whatever the tracker holds outside it: bugs, and decisions that no slice has been charted for. A map's **frontier** — open child issues with no open blocker and no assignee — is how to read one while a slice is being walked, and there is no such slice today.
