...
**Two defects are known and open against what ships.** The first ([#258](https://github.com/algernon28/vespera/issues/258)): a bracket is structural in an ATX heading and in a table cell as much as in link text, and neither the heading rule nor the cell rule guards it — so a document's own title or a model-written cluster title reading `[text](url)` renders as a live link to wherever that url points, and one reading `![alt](https://host/x.png)` renders as an image GitHub fetches through its camo proxy, which is a network request out of a tree ADR-103 says resolves every link with no database, no ledger and no network. **The behaviour is older than the ticket**: ADR-134 guarded the bracket exactly where a link is being formed, which was coherent on what it then knew, and ADR-136 asserted the hazard away — a heading carries only `onOneLine` today, §4 says, "because it has no pipe and no brackets to guard" — while justifying the shape of its own rule. It carries a second false sentence in its consequences: **"`onOneLine`'s callers divide"**, where they did the opposite — `onOneLine` has exactly two callers, `inACell` and `inAHeading`, both of them escaping rules and both reaching a renderer, so the change consolidated them. #258 carries the decision, and the amendments to both sentences travel with it rather than being made in place: `docs/adr/README.md` reopens a record only by a later record. The second ([#259](https://github.com/algernon28/vespera/issues/259)): a membership entry's destination is composed through `java.net.URI`, which quotes a space and a `#` and leaves `&` alone, and six of the seven renderer configurations decode an entity reference in a link destination and the seventh, `marked`, passes it into the `href` untouched, where the browser's HTML attribute parser decodes it instead — so a document named `reports/&copy; notes.pdf` is listed under the name the archive really holds and its link routes to `reports/© notes.pdf` in seven renderer configurations of seven. **The entry names the right document and leads somewhere else**, which is the failure mode hardest for a reviewer to notice, since the page reads correctly and only following the link disagrees with it. It is narrow — the run after the `&` has to look like an entity, so `reports/&copy; notes.pdf` breaks where `reports/R&D notes.pdf` does not — and it **gives no signal when it hits**: no error, no broken-link marker, just a destination naming a document the archive does not hold. ADR-136's `\&` guards the ampersand in an entry's text and was never about its route; the fixture that would have shown the difference carried an invented `%26` through three rounds of measurement, which is why three of them missed it. Three other defects were known and open, and one measurement of how the deliverable actually renders (`docs/research/deliverable-markdown-rendering.md`) closed the last two of them. The first — a citation's ordinal and the membership list's numbering were not the same numbering wherever a member was dropped between the cluster and the call, so a citation could lead to the wrong document ([#236](https://github.com/algernon28/vespera/issues/236)) — is closed by ADR-133: `synthesis`'s fourth table records which documents each call carried under which ordinal, and a cluster file numbers its membership from that record. The second — a membership entry's `file:` link was not a link at all in `markdown-it` or on GitHub, the scheme refused and no anchor emitted, so the filesystem was never asked ([#253](https://github.com/algernon28/vespera/issues/253)) — is closed by ADR-135: an entry links by a path relative to the page that carries it, measured to resolve in seven renderer configurations of seven against two of seven for `file:`, and where no relative path exists it states the document and links nowhere. The third — `<` and `&` reached every renderer unescaped, so a title reading `<b>Retrofits` went live and one reading `<draft>` was deleted outright by GitHub's sanitiser with nothing left to say a word was removed ([#251](https://github.com/algernon28/vespera/issues/251)) — is closed by ADR-136, and **what closed it is a distinction three successive readings of that ticket collapsed**: `\<` and `\&` are backslash escapes, not the HTML entities `&lt;` and `&amp;`, and they behave differently. Writing the entity is wrong, as the ticket's own cost argument and the first measurement both found; writing the backslash is correct in all seven configurations measured, costs a plain-text reader two more characters of a kind ADR-134 already writes, and closes the entity-decoding case of `&copy;` in a value a reader sees as a side effect — in a link destination, where no backslash escape applies at all, the same shape is #259 above. The heading became a fourth surrounding under ADR-134's own rule, being the one position that escaped nothing at all. What none of the three needed in the end is the thing the record still deliberately does not have: which renderers an operator actually opens the tree with, VS Code, IntelliJ and Obsidian among them — every decision above holds in every configuration measured, so none of them had to choose.
...
**`docs/adr/`** holds 136 decisions, ADR-001 to ADR-136, and two things about it are invisible from the files:
